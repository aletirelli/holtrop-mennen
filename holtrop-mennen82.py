...
#AN APPROXIMATE POWER PREDICTION METHOD - J. Holtrop, G.G.J. Mennen
#
# per navi a Fn < 0.4
...
L = 112.45 #lenght waterline [m]
B = 18.5 #[m]
T = 5.400 #m. draught [m]
S = 2685.8 #wetted surface [m^2]
A_M = 98.935 #area sez maestra [m^2]
...
if T/L>=0.05:
    ...
    c_12=48.20*((T/L-0.02)**2.078)+0.479948
else:
    c_12=0.479948
k1 = c_13*(0.93+c_12*((B/L_R)**0.92497)*((0.95-C_P)**-0.521448)*(1-C_P+0.0225*lcb)**0.6906)
R_F=R_F*k1

#APPENDAGE RESISTANCE
k2 = 2.8 #appenages coeff. (solo pinne stabilizzatrici)
...
c_2 = np.exp(-1.89*np.sqrt(c_3)) #riduzione della res onda dovuta al bulbo
c_5 = 1-0.8*A_T/(B*T*C_M) #influenza del transom
if C_P<0.8:
...
if L**3/nabla<=512:
    c_15=-1.69385
elif 512<L**3/nabla<1727:
    c_15=-1.69385+(L/nabla**(1/3)-8.0)/2.36
else:
    c_15=0
m_2 = c_15*C_P**2*np.exp(-0.1*Fn**-2)
d = -0.9
if L/B <=12:
    lmbd = 1.44*C_P-0.03*L/B
else:
    lmbd = 1.446*C_P-0.36
R_W=c_1*c_2*c_5*nabla*rho*g*np.exp(m_1*Fn**d+m_2*np.cos(lmbd*Fn**-2)) #wave resistance [N]
